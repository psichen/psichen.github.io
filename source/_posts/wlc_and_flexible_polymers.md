...
$$
\epsilon = - \frac{(R+y)\theta - R\theta}{R\theta} = - \frac{y}{R} \tag{2}\label{strain}
$$
...
$$
\sigma = E\epsilon = - E \frac{y}{R} \tag{3}\label{stress}
$$
...
$$
M = - \int_{A} \sigma y dA = \frac{E}{R} \int_{A} y^2 dA \tag{4}\label{moment}
$$
...
$$
I = \int_A y^2 dA = \frac{hw^3}{12} \tag{5}\label{inertia}
$$ 
...
$$
\partial_{s}U = \frac{1}{2} \frac{EI}{R^2} = \frac{1}{2} EIc^2 \tag{6}\label{energy}
$$
...
$$
P(\theta) = \exp(-\frac{1}{2} \frac{\kappa}{k_BT s} \theta) / Z \tag{7}\label(dist)
$$
...
$$
<\boldsymbol{t}(0) \cdot \boldsymbol{t}(s)> = <\cos(\theta)> \tag{8}\label{corr}
$$
...
$$
< \cos(\theta) > = \Big{(} \int \exp(-i \omega \theta) P(\theta) d\theta \Big{)}_{\omega=1} = \widetilde{P}(1) = \exp(- \frac{1}{2} \frac{k_BT}{\kappa} s) = \exp(- \frac{s}{2 l_p}) \tag{9}\label{decay}
$$
...
$$
< \cos(\alpha) > = \widetilde{P}(1)^2 = \exp(- \frac{s}{l_p}) \tag{10}\label{decay_3d}
$$
...
So we have,
$$
< \delta \theta^2 > = \frac{\delta s}{l_p} \tag{11}\label{eq1}
$$
...
&= \int \delta \theta^2 \exp(- \frac{1}{2} \frac{\kappa}{k_BT \delta s} \delta \theta^2) d\delta \theta \Big{/} \int \exp(- \frac{1}{2} \frac{\kappa}{k_BT \delta s} \delta \theta^2) d\delta \theta \tag{12}\label{secmom}
\end{align*}
$$
...
$$
M^{(2)} = - \partial_{s}I(1) = \frac{\sqrt{\pi}}{2} \tag{13}\label{secint}
$$
...
$$
< \delta \theta^2 > = (\frac{1}{2} \frac{\kappa}{k_BT \delta s})^{-\frac{3}{2}} \frac{\sqrt{\pi}}{2} \Big{/} (\frac{1}{2} \frac{\kappa}{k_BT \delta s})^{-\frac{1}{2}} \sqrt{\pi} = \frac{k_BT \delta s}{\kappa} \tag{14}\label{eq2}
$$
